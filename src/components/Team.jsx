...
const founders = [
  {
    ...
    accent: 'var(--rose)',
    accentBg: 'rgba(243,39,105,0.12)',
    glowColor: 'rgba(243,39,105,0.25)',
  },
  {
    name: 'Lorenzo Ortiz',
...
export default function Team() {
  return (
    <section id="equipo" className="sec-t on-dark" style={{ background: 'var(--dark)', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Blobs decorativos */}
      <div style={{ position: 'absolute', top: '10%', right: '-8%', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(243,39,105,0.08) 0%, transparent 65%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '5%', left: '-8%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(193,244,0,0.06) 0%, transparent 65%)', pointerEvents: 'none' }} />
      ...
      <div className="wrap" style={{ position: 'relative' }}>
        ...
        <FadeIn delay={0.14}>
          <p className="lead" style={{ color: 'var(--t-white2)', maxWidth: 520, marginBottom: '3rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }} className="team-grid">
          {founders.map((f, i) => (
            <FadeIn key={f.name} delay={0.1 + i * 0.14}>
              <motion.div
                whileHover={{ y: -6 }}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr' }} className="founder-inner">
                  ...
                  <div style={{ position: 'relative', overflow: 'hidden', minHeight: 260 }}>
                    ...
                    <div style={{
                      ...
                      width: 40,
                      background: 'linear-gradient(to right, transparent, rgba(14,14,14,0.85))',
                    }} />
                  </div>
                  ...
                  <div style={{ padding: '1.6rem 1.6rem 1.6rem 1.4rem', display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
                    ...
                    <div>
                      <h3 style={{ fontFamily: 'var(--fout)', fontSize: '1.25rem', fontWeight: 700, color: 'var(--white)', marginBottom: '0.2rem', lineHeight: 1.2 }}>
                        {f.name}
                      </h3>
                      <div style={{ fontFamily: 'var(--fbc)', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: f.accent }}>
                        {f.role}
                      </div>
                    </div>

                    <p style={{ fontSize: '0.83rem', color: 'var(--t-white2)', lineHeight: 1.65 }}>
                      {f.bio}
                    </p>
                    ...
                    <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                      {f.credentials.map((c, ci) => (
                        <li key={ci} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', fontSize: '0.78rem', color: 'var(--t-white3)', lineHeight: 1.5 }}>
                          <span style={{ color: f.accent, fontWeight: 700, flexShrink: 0, marginTop: '0.05rem' }}>—</span>
                          {c}
                        </li>
                      ))}
                    </ul>
                    ...
                    <div style={{ marginTop: 'auto', paddingTop: '0.5rem' }}>
                      <a
                        ...
                        onMouseEnter={e => { e.currentTarget.style.opacity = '1'; e.currentTarget.style.background = f.accentBg.replace('0.10', '0.22').replace('0.12', '0.25') }}
                        onMouseLeave={e => { e.currentTarget.style.background = f.accentBg }}
                      >
                        <LiIcon /> LinkedIn
                      </a>
                    </div>
      ...
      <style>{`
        ...
        @media(max-width:600px){ .founder-inner{ grid-template-columns:1fr!important } .founder-inner img{ height:220px; width:100%; } }
      `}</style>
    </section>
  )
}
